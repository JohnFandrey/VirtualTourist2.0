...
//  VirtualTourist
//
//  Created by John Fandrey on 7/11/18.
...
class FlickrClient: NSObject {
    ...
    func getNumberOfPages(_ pin: Pin, _ completionHandlerForNumberOfPages: @escaping (_ data: AnyObject?, _ error: String?) -> Void) {
        let urlString = "https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=*****************************&bbox=\(pin.minLongitude)%2C\(pin.minLatitude)%2C\(pin.maxLongitude)%2C\(pin.maxLatitude)&safe_search=1&extras=url_m&per_page=100&&format=json&nojsoncallback=1"
        let url = URL(string: urlString)
        let request = URLRequest(url: url!)
        _ = self.taskForMethod(request, completionHandlerForNumberOfPages) // Calls the taskForMethod function and passes the desired completion handler to the function.
    }
    
    
    func getPhotos(_ pin: Pin, _ completionHandlerForGetPhotos: @escaping (_ data: AnyObject?, _ error: String?) -> Void){
        ...
        let urlString = "https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=c920075f890e77cdbda4d46c09537e62&bbox=\(pin.minLongitude)%2C\(pin.minLatitude)%2C\(pin.maxLongitude)%2C\(pin.maxLatitude)&safe_search=1&extras=url_m&per_page=20&page=\(randomPage)&format=json&nojsoncallback=1"
        let url = URL(string: urlString)
        let request = URLRequest(url: url!)
